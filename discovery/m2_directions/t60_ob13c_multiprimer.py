...
def balanced_lp_norm(subset, k):
    ...
    for vals in iproduct(*[range(0, radius + 1)] * n):
        if sum(vals) != k:
            continue
        norm = (
            max(subset[i] * vals[i] for i in range(n))
...
for T in [(), (3,), (5,), (3, 5)]:
    ...
    print(
        f"  T={T}: LP t*={t_star:.3f}, integer_best={int_norm}, alloc={alloc}, norm=max(2,{int_norm})={full_norm}"
    )
...
for size in range(1, 4):
    for T in combinations([3, 5, 17], size):
        ...
        print(
            f"  T={T}: LP t*={t_star:.3f}, integer_best={int_norm}, alloc={alloc}, norm={full_norm}"
        )
...
for p1 in [3, 5, 7, 11]:
    for p2 in [5, 7, 11, 13]:
        ...
        for k in [2, 4, 6, 8]:
            ...
            print(
                f"  (1,{b},{c}){'':<8} {str(nd):>7} {best_T_label:>20} {best_lp:>8.3f} {formula:>8} {match}"
            )
