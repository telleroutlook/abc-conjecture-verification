...
for (a, b, c) in omega2_valid:
    ...
    primes = sorted(set(list(fa.keys()) + list(fb.keys()) + list(fc.keys())))
    p2 = primes[1]
    print(f"  ({a},{b},{c}): formula={nd_f}, brute={nd_b}, p2={p2}, nd>=p2={'YES' if nd_f>=p2 else 'FAIL'} {'OK' if ok else 'MISMATCH'}")
...
for (a, b_raw, c_raw) in lb1_cases:
    ...
    print(f"  ({a},{b},{c}): nd={nd}, p2={p2}, P={primes[:4]}{'...' if len(primes)>4 else ''}, "
...
print("  and the single-prime lemma forces phi_{p1}=0 too. So phi=0, W=0: contradiction.")
