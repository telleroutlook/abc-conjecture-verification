...
for c in range(6, C_MAX+1):
    fc = factorize(c)
    if any(v>1 for v in fc.values()): continue  # squarefree
    omega_c = len(fc)
    if omega_c > 5: continue  # ω(c) ≤ 5
    for a in range(1, (c+1)//2 + 1):
        b = c - a
        if b < a: continue
        if gcd(a,b) != 1: continue
        fa = factorize(a)
        if any(v>1 for v in fa.values()): continue
        fb = factorize(b)
        if any(v>1 for v in fb.values()): continue
        pa = set(fa.keys()); pb = set(fb.keys()); pc = set(fc.keys())
        if pa & pb or pa & pc or pb & pc: continue  # must be coprime
        ...
        if tkey not in records or rho > records[tkey][0]:
            records[tkey] = (rho, a, b, c, tuple(sorted(pa)), tuple(sorted(pb)), tuple(sorted(pc)))
...
for c in range(6, C_MAX+1):
    ...
    for a in range(1, (c+1)//2 + 1):
        ...
        p1, p2 = sorted(fa.keys()); q = list(fb.keys())[0]
        if p1 in (r1,r2) or p2 in (r1,r2) or q in (r1,r2): continue
...
print(f"  Top 10 triples for type (2,1,2):")
...
print(f"  Extremal triple analysis: (6, 1511, 1517) = (2·3, 1511, 37·41)")
...
print(f"  sup_ω=5 ρ < {global_max*1.01:.4f} (1% margin above empirical max {global_max:.6f})")
print()
