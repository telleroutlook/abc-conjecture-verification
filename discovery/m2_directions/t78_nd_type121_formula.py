...
def np0_min_121(j, m, n, q, r, s, search=14):
    ...
    for phi_s in range(-search, search + 1):
        ...
        for t in range(int(t_opt) - 5, int(t_opt) + 6):
            ...
            if phi_q == 0 and phi_r == 0 and phi_s == 0:
                continue
            if norm > 0 and norm < best:
                best = norm
    return best
...
def nd_omega3_min(k, ek, en, pk, pr, ps, search=14):
    ...
    for phi_s in range(-search, search + 1):
        ...
        for t in range(int(t_opt) - 5, int(t_opt) + 6):
            ...
            if phi_p == 0 and phi_r == 0 and phi_s == 0:
                continue
            if norm > 0 and norm < best:
                best = norm
    return best
...
# ---- Main verification ----
print(
    f"T78_formula: 4-branch nd formula for type (1,2,1) triples (LIMIT={LIMIT}, BOUND={BOUND})"
)
print("=" * 75)
...
for a, (p, k) in prime_powers.items():
    for b, (q, r_p, j, m) in two_prime_b.items():
        ...
        if p in (q, r):
            continue
        c = a + b
        if c > 2 * LIMIT or c not in prime_powers:
            continue
        s, n = prime_powers[c]
        if s in (p, q, r):
            continue
...
for a, b, p, q, r, s, k, j, m, n in triples:
    ...
    if f_nd == nd:
        ...
        fail += 1
        print(
            f"FAIL: ({a},{b}) p={p}^{k} q={q}^{j}*{r}^{m}+{s}^{n}: nd={nd} formula={f_nd}"
        )
