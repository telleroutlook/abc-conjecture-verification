...
for r2 in range(5, 200):
    if not isprime(r2) or r2==3: continue
    c = 3*r2; q = c-2
    if not isprime(q) or q==3: continue
...
for c in range(6, 3001):
    if c%2==0: continue  # c-odd only
    fc = factorize(c)
    if not is_squarefree(c) or len(fc)!=2: continue
    r1,r2 = sorted(fc.keys())
    q = c-2
    if q<2 or not isprime(q): continue
    a=2; b=q
    if gcd(a,b)!=1 or gcd(b,c)!=1: continue
    if partition_type(a,b,c) != (1,1,2): continue
    if r1 >= q:
...
for c in range(6, 5001):
    ...
    q=c-2
    if q<2 or not isprime(q): continue
    if gcd(2,q)!=1 or gcd(q,c)!=1: continue
    if partition_type(2,q,c)!=(1,1,2): continue
    R=2*q*c
    nd=nd_norm(2,q,c)
    ratio=nd/R**(1/3)
    if ratio>max_r: max_r=ratio; max_t=(2,q,c)
...
TARGET = 2**(-1/3)
violations=0; max_all=0; max_all_t=None
for c in range(4, 5001):
    for a in range(1,(c+1)//2+1):
        ...
        nd=nd_norm(a,b,c)
        ratio=nd/R**(1/3)
        if ratio>max_all: max_all=ratio; max_all_t=(a,b,c)
        if ratio>TARGET+1e-9:
            violations+=1; print(f"  VIOLATION: ({a},{b},{c}) ρ={ratio:.6f} > 2^{{-1/3}}")
print(f"  Max ρ overall = {max_all:.10f} at {max_all_t}")
print(f"  2^{{-1/3}}      = {TARGET:.10f}")
print(f"  Gap            = {TARGET-max_all:.10f}")
print(f"  Violations: {violations}")
if violations==0: print(f"  All type (1,1,2) triples satisfy ρ < 2^{{-1/3}}. ✓")
print()
...
print(f"      Finite max at (2,13,15): ρ ≈ {3/(2*13*15)**(1/3):.6f} ≪ 2^{{-1/3}}.")
print()
