...
for a in range(2, 600):
    ...
    pa = list(fa.keys())[0]; kval = fa[pa]
    for b in range(1, 600):
        fb = factorize(b)
        if len(fb) != 1: continue
        pb = list(fb.keys())[0]; mval = fb[pb]
        if pb == pa: continue
        c = a + b
        fc = factorize(c)
        if len(fc) != 1: continue
        pc = list(fc.keys())[0]; nval = fc[pc]
...
def is_pairwise(pa, pb, pc, k, m, n):
    ...
    if N0 <= pc: return False, N0, N1, N2
    if N1 <= pa: return False, N0, N1, N2
    if N2 <= pb: return False, N0, N1, N2
...
pl_ok = 0; pl_fail = 0; pure_ok = 0; mixed_other = 0
bezout_condition_verified = 0
bezout_condition_failed = 0

print(f"{'Triple':15s} {'p_L':4s} {'nd':4s} {'nd=pL':6s} {'Bez_norm':8s} {'Bez<=pL':7s} {'Regime':15s}")
print("-" * 75)
...
for (a, b, pa, pb, pc, k, m, n, N0, N1, N2) in pairwise_cases:
    ...
    if pL == pa:
        ...
        for phi_a in range(-20, 21):
            ...
            if norm < bez_best: bez_best = norm
        bez_norm = bez_best
        bez_fits = (bez_norm <= pL)
        regime = "pL=pc"
    ...
    if bez_fits and nd_equals_pL:
        ...
        bezout_condition_failed += 1
        print(f"  BEZ_PRED_FAIL: ({a},{b}) pL={pL} nd={nd_b} bez_norm={bez_norm} (bez>pL but nd=pL!)")
    ...
    print(f"  ({a:3d},{b:3d}):  pL={pL:3d}  nd={nd_b:3d}  {tag:5s}  bez={bez_norm:5.0f}  {"fits" if bez_fits else "NO":4s}  {regime}")
