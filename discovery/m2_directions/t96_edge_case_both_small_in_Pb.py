...
def main():
    ...
    for M in squarefrees:
        ...
        for v in range(3, max_v + 1):
            ...
            # Pb-Pc construction
            fb_dict = {p: v for p in pb_primes}
            p_min = pb_primes[0]
            q_min = pi3
            vp = v
            vq = fc[q_min]
            ...
            edge_cases.append({
                'M': M, 'v': v, 'c': c, 'omega': omega, 'vmax': vmax,
                'R': R, 'bound': round(bound, 2),
                'pi3': pi3, 'pi3^(w-1)': pi3_power_test, 'R>=pi3^(w-1)': pi3_le_R,
                'norm_ub': norm_ub, 'holds': holds,
                'pb_primes': pb_primes, 'pc_primes': pc_primes,
    ...
    if edge_cases:
        print(f"{'M':>4} {'v':>3} {'pi3':>5} {'omega':>5} {'vmax':>5} {'R':>12} {'c':>14} "
              f"{'pi3^(w-1)':>12} {'R>=':>5} {'norm_ub':>8} {'bound':>8} {'OK':>4} {'ratio':>6}")
        print("-"*105)
        for e in edge_cases:
            print(f"{e['M']:>4} {e['v']:>3} {e['pi3']:>5} {e['omega']:>5} {e['vmax']:>5} "
        ...
        if pi3_violations:
            ...
            for e in pi3_violations:
                print(f"  M={e['M']}, v={e['v']}, pi3={e['pi3']}, pi3^(w-1)={e['pi3^(w-1)']}, R={e['R']}")
        ...
        for e in edge_cases[:20]:
            print(f"  (1, {e['M']}^{e['v']}, {e['c']}) Pb={e['pb_primes']} Pc={e['pc_primes'][:5]}"
