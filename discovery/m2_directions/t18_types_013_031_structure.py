...
passed013 = 0; failed013 = 0
max_ratio_013 = 0.0; max_triple_013 = None

print(f"  {'(a,b,c)':>16}  {'r':>5}  {'p1,p2,p3':>12}  {'nd_norm':>7}  {'ratio':>7}  ok?")
print("  " + "-"*65)

for c in range(6, 401):
    if is_prime(c): continue
    fc = factorize(c)
    if len(fc) != 3 or any(v!=1 for v in fc.values()): continue
    pc = sorted(fc.keys())  # c = p1*p2*p3
    p1, p2, p3 = pc

    b = c - 1
    if b < 2 or not is_prime(b): continue
    r = b
    ...
    if ratio > max_ratio_013:
        max_ratio_013 = ratio; max_triple_013 = (a,b,c)

    print(f"  {str((a,b,c)):>16}  {r:>5}  {str(pc):>12}  {nd_norm:>7}  {ratio:>7.4f}  {'✓' if ok else '✗'}")

print()
print(f"  Type (0,1,3): {passed013+failed013} triples, {passed013} passed, {failed013} failed")
print(f"  Max ratio = {max_ratio_013:.4f} at {max_triple_013}")
print(f"  Ratio trend: grows with r (no universal upper bound)")
...
for b in range(6, 201):
    fb = factorize(b)
    if len(fb) != 3 or any(v!=1 for v in fb.values()): continue
    pb = sorted(fb.keys()); p1,p2,p3 = pb
    ...
    print(f"  {str((a,b,c)):>16}  {r:>5}  {str(pb):>12}  {nd_norm:>7}  {ratio:>7.4f}  {'✓' if ok else '✗'}")

print()
print(f"  Type (0,3,1): {passed031+failed031} triples, {passed031} passed, {failed031} failed")
print()
