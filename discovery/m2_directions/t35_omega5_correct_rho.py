...
def f10_nd(a, b, c):
    ...
    group_mins = []
    if fa:
        group_mins.append(min(fa.keys()))
    if fb:
        group_mins.append(min(fb.keys()))
    if fc:
        group_mins.append(min(fc.keys()))
    group_mins.sort()
    if len(group_mins) < 2:
        return None
...
for c in range(6, C_MAX + 1):
    ...
    for a in range(1, (c + 1) // 2 + 1):
        ...
        fa = factorize(a)
        fb = factorize(b)
        if any(v > 1 for v in fa.values()) or any(v > 1 for v in fb.values()):
            continue
        pa = set(fa.keys())
        pb = set(fb.keys())
        pc = set(fc.keys())
...
print("  Note: (0,2,3) with near-equal b-primes approaches rho->1 (sup=1)")
