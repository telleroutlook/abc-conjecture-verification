...
def optimal_nd_norm(a, b, c):
    ...
    candidates = []
    if Pa and Pb: candidates.append(max(min_a, min_b))
    if Pa and Pc: candidates.append(max(min_a, min_c))
    if Pb and Pc: candidates.append(max(min_b, min_c))
...
print()
print(f"{'Type':14}  {'#triples':>8}  {'max_ratio':>10}  {'min_ratio':>10}  {'pattern'}")
print("  " + "-"*65)
for pt in sorted(types_data.keys()):
    ...
    if empties >= 1:
        ...
        if any(n==1 and m==0 for (n,m) in [(na,nb),(nb,na),(nc,na),(na,nc),(nb,nc),(nc,nb)]):
            pred = "UNBOUNDED?"
        else:
            pred = "bounded?"
    elif singletons >= 2:
        ...
        pred = "bounded?"
    print(f"  {str(pt):14}  {len(data):>8}  {max_r:>10.4f}  {min_r:>10.4f}  {pred}  worst=({worst[0]},{worst[1]},{worst[2]})")

print()
print("Top-5 worst ratios across all ω=6 triples:")
all_data = [(a,b,c,r,nd,pair,pt) for pt,data in types_data.items() for (a,b,c,r,nd,pair) in data]
...
for pt, data in types_data.items():
    for (a,b,c,ratio,nd_norm,pair) in data[:5]:
        fa, fb, fc = factorize(a), factorize(b), factorize(c)
        mins = [min(fa.keys()) if fa else float('inf'),
...
if errors == 0:
    print(f"  All verified: F10 formula holds for all ω=6 triples checked. ✓")
