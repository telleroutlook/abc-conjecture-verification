...
def nd4tc(pa, pb, pc1, pc2, ka, kb, n1, n2):
    return min(
        nd111(pa, pb, pc1, ka, kb, n1),
        nd111(pa, pb, pc2, ka, kb, n2),
        max(pb, bm(n1, n2, kb, pc1, pc2)),
        max(pa, bm(n1, n2, ka, pc1, pc2)),
    )
...
for a, (p, q, k1, k2) in two_prime.items():
    for b, (r, s, j1, j2) in two_prime.items():
        ...
        if t in (p, q, r, s) or u in (p, q, r, s) or t == u:
            continue
        if math.gcd(a, b) != 1:
            continue
...
for tt in sample:
    ...
    if f == br:
        ...
        low += 1
        print(f"  low ({a},{b}): f={f} br={br}")
    else:
        fail += 1
        print(f"  FAIL ({a},{b}): f={f} br={br}")
