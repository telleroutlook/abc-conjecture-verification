        ...
        for phi_r in range(-search, search + 1):
            for phi_q in range(-search, search + 1):
                ...
                if rhs % k1 != 0:
                    continue
                phi_p = rhs // k1
                if phi_p + phi_q + phi_r == 0:
                    continue
                if phi_t == 0 and phi_p + phi_q + phi_r == 0:
                    continue
                norm = max(
                    p * abs(phi_p), q * abs(phi_q), r * abs(phi_r), t * abs(phi_t)
                )
                if norm > 0 and norm < best:
                    best = norm
    return best
...
print(
    f"T82: type (3,1,1) exact nd verification — smart bounded verifier (LIMIT={LIMIT})"
)
print("=" * 70)
...
for a, b, p, q, r, s, t, k1, k2, k3, m, n in triples:
    ...
    if tight:
        ...
        fail += 1
        print(
            f"FAIL: ({a},{b}) p={p}^{k1}*q={q}^{k2}*r={r}^{k3}+s={s}^{m}=t={t}^{n}: "
