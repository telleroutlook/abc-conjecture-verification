...
def nd_formula(p, q, r, s, k1, k2, m, nv):
    ...
    B2 = min(max(p*m//g1m, r*k1//g1m), max(q*m//g2m, r*k2//g2m),
             max(p*nv//g1n, s*k1//g1n), max(q*nv//g2n, s*k2//g2n),
...
for n in range(2, LIMIT + 1):
    ...
    if len(f) == 2:
        ...
        p = list(f.keys())[0]; prime_powers[n] = (p, f[p])
...
branch_names = ['B1_intra', 'B2_cross', 'B3_zero_q', 'B4_zero_p', 'B5_zero_r', 'B6_zero_s']
...
sample_ok = 0; sample_fail = 0; sample_low = 0
...
for (a, b, p, q, r, s, k1, k2, m, nv) in sample:
    ...
    if formula == brute:
        ...
        sample_low += 1
        print(f"  FORMULA<BRUTE ({a},{b}): formula={formula} brute(bd={BRUTE_BOUND})={brute}")
    else:
        sample_fail += 1
        print(f"  FAIL ({a},{b}): formula={formula} brute={brute}")

print(f"Spot-check ({len(sample)} triples, brute bound={BRUTE_BOUND}):")
print(f"  OK: {sample_ok},  formula<brute(bound too small): {sample_low},  formula>brute(FAIL): {sample_fail}")
print()
...
if sample_fail == 0:
    ...
    print("  nd(a,b) = min(B1_intra*, B2_cross, B3_zero_q, B4_zero_p, B5_zero_r, B6_zero_s)")
