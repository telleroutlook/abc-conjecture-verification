...
def find_min_nondeg(a, b, c, primes, coeff, fa, fb, fc, bound=60):
    ...
    min_nd = None; min_d = None; nd_vec = None
      ...
      for f2 in range(-max_phi, max_phi+1):
        for f3 in range(-max_phi, max_phi+1):
          for f4 in range(-max_phi, max_phi+1):
            ...
            sign = {}
            for p in fa: sign[p] = sign.get(p,0)+1
            for p in fb: sign[p] = sign.get(p,0)+1
            for p in fc: sign[p] = sign.get(p,0)-1
            # sign_p5*f5 = -(sign_p1*f1+...+sign_p4*f4)
            sp5 = sign.get(p5,0)
            if sp5 == 0: continue
            rhs = -(sign.get(p1,0)*f1+sign.get(p2,0)*f2+sign.get(p3,0)*f3+sign.get(p4,0)*f4)
            if rhs % sp5 != 0: continue
            f5 = rhs // sp5
            phi = {p1:f1, p2:f2, p3:p3, p4:f4, p5:f5}
            phi = {p1:f1, p2:f2, p3:f3, p4:f4, p5:f5}
            if all(v==0 for v in phi.values()): continue
            psi = {q: q*phi[q] for q in primes}
            norm = max(abs(q*phi[q]) for q in primes)
            if norm > bound: continue
            W = wronskian(a, b, psi, fa, fb)
            if abs(W) > 1e-9:  # non-degen
                if min_nd is None or norm < min_nd:
                    min_nd = norm; nd_vec = dict(phi)
            else:  # degen
...
for c in range(6, 201):
    for a in range(2, (c+1)//2+1):
        ...
        min_nd, min_d, nd_vec = find_min_nondeg(a,b,c,primes,coeff,fa2,fb2,fc2, bound=50)

        is_degen = (min_d is not None and (min_nd is None or min_d < min_nd))
        ratio_nd = min_nd / R**(1.0/4) if min_nd is not None else None  # R^{1/(ω-1)} = R^{1/4}

        key = tp
        if key not in type_stats:
            type_stats[key] = {'total':0,'nd_count':0,'degen_count':0,'ratios':[],'pred_ok':0,'pred_fail':0}
...
print("="*75)
print()
print(f"  {'Type':>12}  {'Total':>6}  {'Degen':>6}  {'ND':>5}  {'MaxRatioND':>11}  {'PredOK/Fail':>11}  {'Pattern'}")
print("  " + "-"*80)
...
for tp in sorted(type_stats.keys()):
    ...
    pattern = "ALL_DEGEN" if all_degen else ("BOUNDED" if bounded else "UNBOUNDED" if max_r > 1.2 else "?")
    print(f"  {str(tp):>12}  {s['total']:>6}  {s['degen_count']:>6}  {s['nd_count']:>5}  {max_r:>11.4f}  {pred_str:>11}  {pattern}")
...
seen_types = set()
for (a,b,c,tp,min_nd,min_d,ratio_nd,pred_nd) in sorted(all_triples, key=lambda x:(x[3],x[2])):
    if tp in seen_types: continue
    seen_types.add(tp)
    fa = factorize(a); fb = factorize(b); fc = factorize(c)
    pa, pb, pc = sorted(fa.keys()), sorted(fb.keys()), sorted(fc.keys())
    R = 1
    for p in sorted(set(fa)|set(fb)|set(fc)): R *= p
    ...
    if ratio_nd: print(f"    nd_ratio={ratio_nd:.4f}  degen_overall={degen}")
    print()
