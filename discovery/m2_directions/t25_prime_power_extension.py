...
print()
print(
    f"  {'triple':>30}  {'ω':>3}  {'type':>9}  {'R':>8}  {'nd':>5}  {'quality':>8}  {'ratio':>8}  {'class'}"
)
print("  " + "-" * 85)
...
for a, b, c, desc, q_src in valid_triples:
    ...
    if all(n <= 2 for n in (na, nb, nc)):
        ...
        cls = "UNBND"
    print(
        f"  {desc:>30}  {om:>3}  {str(pt):>9}  {R:>8}  {nd:>5}  {quality:>8.4f}  {ratio:>8.4f}  {cls}"
    )
...
quals = [
    (
        math.log(c) / math.log(rad_abc(a, b, c)),
        optimal_nd_norm(a, b, c)
        / rad_abc(a, b, c) ** (1.0 / max(omega(a, b, c) - 1, 1)),
    )
    for (a, b, c, _, __) in valid_triples
    if rad_abc(a, b, c) > 1
]
...
for n in [3, 5, 7, 13, 17]:
    ...
    # Check Mersenne
    is_m = (
        all(b_cand % d != 0 for d in range(2, int(b_cand**0.5) + 1))
        if b_cand > 1
        else False
    )
    if not is_m:
    ...
    print(
        f"  n={n:2}: (1, {b_cand:6}, {c_cand:6})  type={pt}  R={R:6}  q={quality:.4f}  nd={nd}  ratio={ratio:.4f}"
    )
...
print()
print(
    "  3. The F-series bounds are NOT the binding constraint for high-quality triples."
)
