...
def make_triples(na, nb, nc, max_t=10):
    ...
    for pa_c in gen_k_primes(na, set(), limit=5):
        ...
        for pb_c in gen_k_primes(nb, set(pa_c), limit=5):
            ...
            if len(triples) >= max_t: return triples
    return triples
...
for na, nb, nc in omega6_types:
    ...
    for pa, pb, pc in triples[:6]:
        ...
        if best != nd:
            all_match = False
            total_ok = False
            print(f"  MISMATCH type ({na},{nb},{nc}): {pa+pb+pc}, nd={nd}, min={best}")
...
for na, nb, nc in omega6_types:
    ...
    if v is None: status = "no triples"
    elif v: status = "✓"
    else: status = "✗ FAILS"
...
print("="*72)
if total_ok:
    print(f"✓ ALL ω=6 types: min_nondeg_norm = F10's nd")
    print(f"E_n conjecture confirmed for ω=3,4,5,6.")
else:
