...
def bezout_min_norm(m, n, k, pb, pc):
    ...
    for sign in [+1, -1]:
        ...
        for t in range(int(t_opt) - 6, int(t_opt) + 7):
            phi_b = u_part + step_u * t; phi_c = v_part + step_v * t
...
def nd_111(pa, pb, pc, k, m, n):
    ...
    N0=max(pa*m//g_km, pb*k//g_km); N1=max(pb*n//g_mn, pc*m//g_mn); N2=max(pa*n//g_kn, pc*k//g_kn)
    if N0<=pc: return N0
    if N1<=pa: return N1
    if N2<=pb: return N2
    pL=max(pa,pb,pc)
    if pL==pa: B=bezout_min_norm(m,n,k,pb,pc)
    elif pL==pb: B=bezout_min_norm(k,n,m,pa,pc)
    else:
        B=float('inf')
        for phi_c in [+1,-1]:
            rhs=n*phi_c
