        ...
        for T in combinations(Pb, size):
            ...
            for vals in iproduct(*[range(0, radius + 1)] * len(T)):
                if sum(vals) != k:
                    continue
                norm = (
                    max(T[i] * vals[i] for i in range(len(T)))
...
print("=" * 110)
print(
    f"{'(a,b,c)':<16} {'sq':>3} {'v_max':>6} {'R':>5} {'nd':>5} {'W_phi':>7} {'W_psi':>8} "
...
for a, b in test_cases:
    ...
    print(
        f"  ({a},{b},{c}){'':<5} {sq_str:>3} {v_max:>6} {R:>5} {nd:>5} "
...
print(
    f"  nd  | W_psi : {divisibility_stats['nd_divides_Wpsi']}/{n}  "
    f"({'ALL' if divisibility_stats['nd_divides_Wpsi'] == n else 'NOT ALL'})"
)
print(
    f"  v_max | W_psi: {divisibility_stats['vmax_divides_Wpsi']}/{n}  "
...
if gap_cases:
    ...
    print(
        f"  {'(a,b,c)':<18} {'brute nd':>8} {'GCD norm':>9} {'gap':>5} {'best pair':>16}"
    )
...
for a, b in test_cases:
    ...
    if not found:
        print(
            f"  ({a},{b},{c}): W_psi={W_psi}, nd={nd}, W_phi={W_phi} → [no simple formula]"
        )
