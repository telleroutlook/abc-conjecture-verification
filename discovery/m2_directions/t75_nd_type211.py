...
for a in range(4, 500):
    fa = factorize(a)
    if len(fa) != 2:
        continue  # exactly 2 distinct primes in a
...
# Analyze each triple
print(
    f"{'Triple':15s} {'a decomp':15s} {'b':6s} {'c':8s} {'nd':4s} {'W_ab':5s} {'N_pure_min':10s} {'winner':20s}"
)
print("-" * 90)
...
for a, b, p, q, r, s, k1, k2, m, nv in triples:
    ...
    print(
        f"  ({a:3d},{b:3d}) {p}^{k1}*{q}^{k2}+{r}^{m}={s}^{nv}  "
...
errors = [
    (a, b, nd, overall)
    for (a, b, p, q, r, s, k1, k2, m, nv, nd, W, N, w) in cases
    for overall in [min(W, N)]
    if nd != overall
]
...
wg_wins = [
    (a, b, p, q, k1, k2, nd, W_ab, N_pure_min)
...
for row in wg_wins[:8]:
    ...
    g = math.gcd(k1, k2)
    print(
        f"  ({a},{b}) {p}^{k1}*{q}^{k2}: g={g}, k2/g={k2 // g}, k1/g={k1 // g}  nd={nd}=max({p}*{k2 // g},{q}*{k1 // g})"
    )

print()
print("=== PURE-BRANCH DOMINANCE ===")
pure_wins = [
    (a, b, p, q, r, s, k1, k2, m, nv, nd, W_ab, N_pure_min, winner)
...
for row in pure_wins[:10]:
    ...
    print(
        f"  ({a},{b}) {p}^{k1}*{q}^{k2}+{r}^{m}={s}^{nv}: nd={nd}<Wab={W}, winner={winning}, branches={dict(branches)}"
    )
...
for a, b, p, q, r, s, k1, k2, m, nv, nd, W_ab, N_pure_min, w in cases:
    if nd < W_ab:
        ...
        g = math.gcd(k1, k2)
        print(
            f"  ({a},{b}) Wab={W_ab} N_pure={N_pure_min} ratio={ratio:.2f} best={best_branch}"
        )
