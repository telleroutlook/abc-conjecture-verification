...
cases = [
    ...
    (3,  4,  7,  2,  3,  7,  1,  2),   # a=3=3^1, b=4=2^2 → P_a={3}, P_b={2}, p=2<q=3<r=7? No: p=2,q=3 but a=3,b=4, Pa={3}, Pb={2}, not p<q ordering by value vs by role
    # Reorient: P_a={smaller prime in a}, P_b={smaller prime in b}
    # Actually P_a is the set of primes in a. Let me just list valid ones:
    (3,  4,  7,  2,  3,  7,  2,  1),   # a=3^1=3, b=2^2=4: P_a={3},P_b={2},P_c={7}, k_3=1,k_2=2... need p=min(P)=2<q=3<r=7 with role P_b={2^2}, P_a={3^1}: p=2,q=3,k=2,m=1 → nd=min(7,max(2,6))=6 → covered by nd_k11
    # k=1,m=2 with p<q<r and P_b={q^2}:
    (2,  9,  11, 2,  3,  11, 1,  2),   # a=2,b=9=3^2,c=11: p=2,q=3,r=11,k=1,m=2,g=1 → nd=min(11,max(4,3))=4
...
for row in cases:
    ...
    fa, fb, fc = factorize(a), factorize(b), factorize(c)
    # Check structure
    if set(fa.keys()) != {p}: continue
    if set(fb.keys()) != {q}: continue
    if set(fc.keys()) != {r}: continue
    if fa[p] != k: continue
    if fb[q] != m: continue
    if fc[r] != 1: continue
...
for (a, b, c, p, q, r, k, m) in valid_cases:
    ...
    print(f"  ({a},{b},{c}): p={p}^{k},q={q}^{m},r={r}  g={g}  "
