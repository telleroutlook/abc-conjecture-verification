...
for omg_target in [2, 3, 4, 5]:
    ...
    print(
        f"  {'(a,b,c)':>22}  {'||ψ||':>7}  {'c^0.5':>7}  {'R':>8}  {'η_c':>6}  {'η_R':>6}  {'q':>6}"
    )
    print("  " + "-" * 70)
    ...
    for a, b, c in valid:
        ...
        if min_norm is None:
            print(
                f"  {str((a, b, c)):>22}  {'N/A':>7}  {c**0.5:>7.2f}  {R:>8}  {'N/A':>6}  {'N/A':>6}  {q:>6.3f}"
            )
            continue
        ...
        print(
            f"  {str((a, b, c)):>22}  {min_norm:>7}  {c**0.5:>7.2f}  {R:>8}  {eta_c:>6.3f}  {eta_R:>6.3f}  {q:>6.3f}"
        )
    print()
...
print()
print(
    f"  {'omega':>5}  {'count':>5}  {'min_η_c':>8}  {'mean_η_c':>9}  {'min_η_R':>8}  {'mean_η_R':>9}"
)
print("  " + "-" * 55)
for omg in [2, 3, 4, 5]:
    ...
    if not data:
        continue
    ec = [x[0] for x in data]
    er = [x[1] for x in data]
    print(
        f"  {omg:>5}  {len(data):>5}  {min(ec):>8.3f}  {sum(ec) / len(ec):>9.3f}  {min(er):>8.3f}  {sum(er) / len(er):>9.3f}"
    )
...
if omega3_R:
    ...
    print(
        f"  omega=3: η_R < 0.5 in {100 * below_half:.0f}% of cases (n={len(omega3_R)})"
    )
    print(
        f"           min η_R = {min(omega3_R):.3f}, mean = {sum(omega3_R) / len(omega3_R):.3f}"
    )
if omega4_R:
    below_half = sum(1 for x in omega4_R if x < 0.5) / len(omega4_R)
    print(
        f"  omega=4: η_R < 0.5 in {100 * below_half:.0f}% of cases (n={len(omega4_R)})"
    )
    print(
        f"           min η_R = {min(omega4_R):.3f}, mean = {sum(omega4_R) / len(omega4_R):.3f}"
    )
