...
def nd_brute_full(a, b, bound=BB):
    ...
    if len(primes) != 6: return None, None, None
...
for a in range(2, LIMIT + 1):
    ...
    for b in range(2, LIMIT + 1):
        ...
        fc = factorize(c)
        primes = sorted(set(list(fa) + list(fb) + list(fc)))
        if len(primes) != 6: continue
        if len(set(list(fa)) & set(list(fb))) > 0: continue
        if len(set(list(fa)) & set(list(fc))) > 0: continue
        if len(set(list(fb)) & set(list(fc))) > 0: continue
...
print(f"T85: omega*=6 zero-coordinate property — {len(triples)} total triples (a,b <= {LIMIT})")
