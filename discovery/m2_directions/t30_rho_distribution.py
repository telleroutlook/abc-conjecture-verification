...
print()
print(f"  {'ω':>3}  {'count':>7}  {'mean ρ':>8}  {'max ρ':>8}  {'%>0.5':>6}  {'%>1.0':>6}  "
      f"{'sharp bound':>12}")
...
for omega in sorted(omega_data.keys()):
    ...
    print(f"  {omega:>3}  {n:>7}  {mean_r:>8.4f}  {max_r:>8.4f}  {pct05:>5.1f}%  "
...
for pt, sharp in sorted(bounded_types.items()):
    ...
    print(f"  Type {pt}: n={n:>5}, mean={mean_r:.4f}, max={max_r:.6f}, "
...
for pt in unbounded_types:
    ...
    if not rhos_with_triples: continue
...
combined_rhos = sorted([r for pt in [(0,2,2),(1,1,2),(1,2,1)]
                        for r in type_data[pt]])
if combined_rhos:
    n = len(combined_rhos)
