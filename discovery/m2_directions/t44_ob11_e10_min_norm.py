...
print(
    f"Found {len(triples)} omega=3 squarefree prime triples (p<q, r=p+q prime) up to p,q<200"
)
print()
...
for p, q, r in triples[:20]:  # Show details for first 20
    ...
    print(
        f"  v3=({p},-{q},0) in L, norm={min_norm_v3}, W={w3} {'≠0 NON-DEG' if w3 != 0 else 'DEG!'}"
    )
    print(
        f"  brute-force min norm={actual_min} at {actual_min_vec}, W={w_actual} {'≠0 NON-DEG' if w_actual != 0 else 'DEG!'}"
    )
    print(
        f"  OB-11 correct? {ob11_claim == actual_min}  |  F10 correct? {f10_pred == actual_min}"
    )
    print()
...
print(
    f"  v=(p,-q,0) in L, norm=q<r, non-degenerate: CONFIRMED for all {ob11_wrong} triples"
)
print(
    "  OB-11 Step 1 ('min norm = r') is WRONG for all triples (norm = q = max(p,q) < r)"
)
print()
...
print(
    "     gcd(p,q)=1 => p|psi_p. Write psi_p=p*t, psi_q=-q*t. Min |t|=1 => norm=max(p,q)=q."
)
print("  2. For psi_r≠0: r|psi_r (from gcd(r,pq)=1) => |psi_r|>=r > q.")
print("     So all psi with psi_r≠0 have norm > q.")
print(
    "  3. Global min norm = q, achieved by (p,-q,0). Wronskian = p(-q)-q(p) = -2pq ≠ 0."
)
