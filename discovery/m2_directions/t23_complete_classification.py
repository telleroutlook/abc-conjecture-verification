...
for c in range(4, 501):
    for a in range(1, (c + 1) // 2 + 1):
        ...
        if omega not in bounded_types:
            continue
        if pt not in bounded_types[omega]:
            continue
        R = rad(a, b, c)
...
for omega in [3, 4, 5]:
    for pt in bounded_types[omega]:
        ...
        print(
            f"Type ω={omega} {str(pt):10}: n={n:4}, max_ratio={max_r:.6f}  ({bound_str})"
        )
        print(
            f"  worst: a={worst[0]}, b={worst[1]}, c={worst[2]}, nd={worst[4]}, ratio={worst[3]:.6f}"
        )
...
if violations == 0:
    print(
        f"  All {len(data.get((4, (1, 2, 1)), []))} triples satisfy ratio ≤ 2^{{-1/3}} = {BOUND:.6f}. ✓"
    )
...
print(
    f"  ω=3 (1,1,1): ratio ≤ √(7/6)={math.sqrt(7 / 6):.4f}  (proved F3; conjectured sharp 1/√2)"
)
print(
    f"  ω=4 (0,2,2): ratio → 0; sup ≤ {max(e[3] for e in data.get((4, (0, 2, 2)), [0, 0, 0, 0, 0])):.4f}  (numerical)"
)
print(
    f"  ω=4 (1,1,2): ratio → 0; sup ≤ {max(e[3] for e in data.get((4, (1, 1, 2)), [0, 0, 0, 0, 0])):.4f}  (numerical)"
)
print(
    f"  ω=4 (1,2,1): ratio ≤ 2^(-1/3)={2 ** (-1 / 3):.4f}  (proved F7a; sharp? gap={2 ** (-1 / 3) - max(e[3] for e in data.get((4, (1, 2, 1)), [0, 0, 0, 0, 0])):.5f})"
)
print(
    f"  ω=5 (1,2,2): ratio → 0; sup ≤ {max(e[3] for e in data.get((5, (1, 2, 2)), [0, 0, 0, 0, 0])):.4f}  (numerical)"
)
print(
    f"  ω=5 (2,1,2): ratio → 0; sup ≤ {max(e[3] for e in data.get((5, (2, 1, 2)), [0, 0, 0, 0, 0])):.4f}  (numerical)"
)
print(
    f"  ω=5 (2,2,1): ratio → 0; sup ≤ {max(e[3] for e in data.get((5, (2, 2, 1)), [0, 0, 0, 0, 0])):.4f}  (numerical)"
)
print()
