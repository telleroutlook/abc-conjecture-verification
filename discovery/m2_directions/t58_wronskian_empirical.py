...
for a in range(2, 120):
    ...
    for b in range(a + 1, 300):
        ...
        if math.prod(pc) != c:
            continue
        # No shared primes
        if set(pa) & set(pb) or set(pa) & set(pc) or set(pb) & set(pc):
            continue
...
print(
    f"H1 (|W_ψ| ≤ ω·nd): {'PASS' if not h1_fails else f'FAILS for {len(h1_fails)} triples'}"
)
...
print(
    f"{'Type':<12} {'count':>6} {'W_ψ/c range':>20} {'|W_ψ|/nd range':>18} {'W_ψ encodes?':>15}"
)
print("-" * 75)
...
print(
    f"{'Type':<12} {'min |W_ψ|/rad':>15} {'max |W_ψ|/rad':>15} {'min |W_ψ|/√rad':>16}"
)
print("-" * 60)
for typ in sorted(by_type.keys()):
    group = by_type[typ]
    ...
    print(
        f"{typ:<12} {min(ratios1):>15.6f} {max(ratios1):>15.6f} {min(ratios2):>16.6f}"
    )
