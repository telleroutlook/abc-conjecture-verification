...
for c in range(6, 2001):
    for a in range(1, (c+1)//2+1):
        ...
        if pt not in omega5_bounded: continue
...
print(f"  nd=3 FIXED, ρ = 3/(30*q₂*r₂)^{{1/4}} → 0")
...
for q2 in range(7, 100000):
    if not isprime(q2) or q2 in {2,3}: continue
    b=3*q2
    c_val=2+b
    if not is_squarefree(c_val): continue
    fc=factorize(c_val)
    if set(fc.keys()) != {5}|{r for r in fc if r>5}: continue  # need 5|c
    if 5 not in fc: continue
    r2=c_val//5
    if not isprime(r2): continue
    if mgcd(2,b)!=1 or mgcd(b,c_val)!=1: continue
    R=2*3*5*q2*r2
    nd=3
    ratio=nd/R**0.25
...
print(f"  nd=3 FIXED, ρ = 3/(30*p₂*r₂)^{{1/4}} → 0")
print(f"  {'p₂':>8}  {'r₂':>8}  {'c':>10}  {'ρ':>10}")
count=0
for p2 in range(7, 100000):
    if not isprime(p2) or p2 in {2,3,5}: continue
    a=2*p2
    c_val=a+3
    if not is_squarefree(c_val) or 5 not in factorize(c_val): continue
    fc=factorize(c_val)
    r2=c_val//5
    if not isprime(r2): continue
    if mgcd(a,3)!=1 or mgcd(3,c_val)!=1: continue
    R=2*p2*3*5*r2
    nd=3
    ratio=nd/R**0.25
...
print(f"  nd=3 FIXED, ρ = 3/(2*p₂*3*q₂*r)^{{1/4}} → 0")
print(f"  {'p₂':>8}  {'q₂':>8}  {'r':>10}  {'ρ':>10}")
count=0
for p2 in range(7, 100000):
    if not isprime(p2) or p2 in {2,3}: continue
    for q2 in range(5, 200):
        if not isprime(q2) or q2 in {2,3}: continue
        a=2*p2; b=3*q2; c_val=a+b
        if mgcd(a,b)!=1: continue
        if not is_squarefree(c_val) or not isprime(c_val): continue
        if len(set(factorize(a))|set(factorize(b))|set(factorize(c_val)))!=5: continue
        R=2*p2*3*q2*c_val
        nd=3
        ratio=nd/R**0.25
...
print(f"  (1,2,1): sup = 2^{{-1/3}} = {2**(-1/3):.6f}  [never achieved, approached ∞]")
print(f"  (1,1,2): sup = 2^{{-1/3}} = {2**(-1/3):.6f}  [never achieved, approached ∞]")
print(f"  (0,2,2): max = 3·210^{{-1/3}} = {3*210**(-1/3):.6f}  [achieved at (1,14,15)]")
print()
