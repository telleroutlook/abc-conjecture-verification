...
for a, b, c in TRIPLES:
    ...
    if min_norm is None or min_norm == 0:
        print(
            f"  {str((a, b, c)):>22}  {omega:>2}  {'N/A':>6}  {R:>8}  "
    ...
    print(
        f"  {str((a, b, c)):>22}  {omega:>2}  {min_norm:>6}  {R:>8}  "
...
print()
print(
    f"  {'ω':>2}  {'n':>3}  {'min_ratio':>9}  {'max_ratio':>9}  {'mean_ratio':>10}  "
...
for omg in [2, 3, 4, 5]:
    ...
    print(
        f"  {omg:>2}  {len(data):>3}  {min(ratios):>9.3f}  {max(ratios):>9.3f}  "
...
print()
print(
    "  det(L) = ||c||_2 / gcd(c_p).  By Minkowski: ||psi||_min <= det(L)^{1/(omega-1)}."
)
print(
    "  If det(L) <= C*R, then ||psi||_min <= (C*R)^{1/(omega-1)} = C^{1/(omega-1)} * R^{1/(omega-1)}."
)
print()
...
for omg in [3, 4]:
    ...
    print(
        f"  omega={omg}: det(L)/R  min={min(ratios):.2f}  max={max(ratios):.2f}  "
        f"mean={sum(ratios) / len(ratios):.2f}"
    )
    print(
        f"           det(L)^{{1/{omg - 1}}} / R^{{1/{omg - 1}}} = "
...
print(
    "  Minkowski gives: ||psi|| <= (max_v * R)^{1/(omega-1)} * omega^{1/(2(omega-1))}."
)
