...
degenerate_cases = []
all_cases = []
max_ratio = 0.0; max_ratio_triple = None
...
for c in range(4, 301):
    for a in range(1, (c+1)//2 + 1):
        ...
        if key not in partition_stats:
            partition_stats[key] = {'count':0,'degen':0,'max_ratio':0.0,'max_triple':None}
...
for ptype in sorted(partition_stats.keys()):
    ...
    t = s['max_triple']
    print(f"  {str(ptype):>10}  {s['count']:>6}  {s['degen']:>6}  {s['max_ratio']:>10.4f}  {t}")
print()
...
if not degenerate_cases:
    ...
    print(f"  {'(a,b,c)':>18}  {'R':>8}  {'type':>10}  {'na':>5}  {'nd':>5}  {'ratio':>8}")
    print("  " + "-"*65)
    for a,b,c,R,ptype,na,nd,ratio in degenerate_cases:
        ratio_s = f"{ratio:.4f}" if ratio else "N/A"
        print(f"  {str((a,b,c)):>18}  {R:>8}  {str(ptype):>10}  {na:>5}  {str(nd) if nd else 'N/A':>5}  {ratio_s:>8}")
print()
