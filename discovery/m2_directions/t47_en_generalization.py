...
def find_min_nondeg(pa_primes, pb_primes, pc_primes, bound=6):
    ...
    # Enumerate all phi vectors with n-1 free vars (last pc var determined by constraint)
    free_indices = list(range(n - 1))  # all except last pc prime
    last_idx = n - 1  # index of last pc prime (r2 for type (1,1,2), etc.)
...
for pa, pb, pc in triples_112[:20]:
    ...
    if best != nd: all_match_112 = False
    print(f"  ({pa[0]:2},{pb[0]:3},{pc[0]:2},{pc[1]:3}): nd={nd:3d} min={best:3d} φ={phi} {ok}")
...
for p in primes[:10]:
    for q1 in primes:
        ...
        for q2 in primes:
            if q2 <= q1: continue
            if q1 == p or q2 == p: continue
            b = q1 * q2
            c = p + b
            if c not in prime_set: continue
            if c == p or c == q1 or c == q2: continue
            triples_121.append(([p], [q1, q2], [c]))
            if len(triples_121) >= 30: break
        if len(triples_121) >= 30: break
    if len(triples_121) >= 30: break
...
for pa, pb, pc in triples_121[:15]:
    ...
    best, phi = find_min_nondeg(pa, pb, pc, bound=5)
    ok = "✓" if best == nd else f"✗(nd={nd})"
    if best != nd: all_match_121 = False
    print(f"  ({pa[0]:2},{pb[0]:2},{pb[1]:3},{pc[0]:4}): nd={nd:3d} min={best:3d} φ={phi} {ok}")
...
for p1 in primes[:8]:
    for p2 in primes:
        ...
        a = p1 * p2
        for q in primes:
            if q == p1 or q == p2: continue
            c = a + q
            if c not in prime_set: continue
            if c == p1 or c == p2 or c == q: continue
            triples_211.append(([p1, p2], [q], [c]))
            if len(triples_211) >= 30: break
        if len(triples_211) >= 30: break
    if len(triples_211) >= 30: break
...
for pa, pb, pc in triples_211[:15]:
    ...
    best, phi = find_min_nondeg(pa, pb, pc, bound=5)
    ok = "✓" if best == nd else f"✗(nd={nd})"
    if best != nd: all_match_211 = False
    print(f"  ({pa[0]:2},{pa[1]:3},{pb[0]:3},{pc[0]:4}): nd={nd:3d} min={best:3d} φ={phi} {ok}")
...
if all_ok:
    ...
    print("  min non-degenerate Pasten norm = second_smallest{min(Pa), min(Pb), min(Pc)} = F10's nd.")
    print()
    ...
    print()
    print("General pattern: optimal vector uses TWO SMALLEST primes from different sides")
