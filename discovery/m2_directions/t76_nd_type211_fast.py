...
# Analyze each triple
print(
    f"{'(a,b)':12s} {'decomp':18s} {'nd':4s} {'Wab':4s} {'nd vs Wab':10s} {'ratio':6s}"
)
print("-" * 60)
...
for a, b, p, q, r, s, k1, k2, m, nv in triples:
    ...
    print(
        f"  ({a:3d},{b:3d})   {p}^{k1}*{q}^{k2}+{r}^{m}={s}^{nv}   nd={nd:3d}  Wab={W_ab:3d}  {rel}  {ratio:.3f}"
    )

print()
print("=" * 80)
print(
    f"Total: {len(cases)}, nd<Wab: {stats['nd<Wab']}, nd=Wab: {stats['nd=Wab']}, nd>Wab: {stats['nd>Wab']}"
)
...
for a, b, p, q, r, s, k1, k2, m, nv, nd, W_ab, g, ratio in cases:
    if nd < W_ab:
        ...
        g1m = math.gcd(k1, m)
        nd_pair_1 = max(p * m // g1m, r * k1 // g1m)
        g2m = math.gcd(k2, m)
        nd_pair_2 = max(q * m // g2m, r * k2 // g2m)
        # Also (p^k1, s^n) pair:
        g1n = math.gcd(k1, nv)
        nd_pair_3 = max(p * nv // g1n, s * k1 // g1n)
        g2n = math.gcd(k2, nv)
        nd_pair_4 = max(q * nv // g2n, s * k2 // g2n)
        # Also (r^m, s^n) pair:
        gmn = math.gcd(m, nv)
        nd_pair_5 = max(r * nv // gmn, s * m // gmn)
        print(
            f"    pairwise norms: ({p},{r})={nd_pair_1}, ({q},{r})={nd_pair_2}, "
            f"({p},{s})={nd_pair_3}, ({q},{s})={nd_pair_4}, ({r},{s})={nd_pair_5}"
        )
        print(
            f"    min_pairwise = {min(nd_pair_1, nd_pair_2, nd_pair_3, nd_pair_4, nd_pair_5)}, nd={nd}"
        )
...
for a, b, p, q, r, s, k1, k2, m, nv, nd, W_ab, g, ratio in cases:
    ...
    g2n = math.gcd(k2, nv)
    gmn = math.gcd(m, nv)
    nd_1 = max(p * m // g1m, r * k1 // g1m)
    nd_2 = max(q * m // g2m, r * k2 // g2m)
    nd_3 = max(p * nv // g1n, s * k1 // g1n)
    nd_4 = max(q * nv // g2n, s * k2 // g2n)
    ...
    if formula == nd:
        ...
        formula_fail += 1
        print(
            f"  FAIL: ({a},{b}) nd={nd} formula={formula} "
...
for a, b, p, q, r, s, k1, k2, m, nv, nd, W_ab, g, ratio in cases:
    if nd < W_ab:
        ...
        for coords in iproduct(range(-bound, bound + 1), repeat=np_):
            ...
            if norm == nd:
                print(
                    f"  ({a},{b}) nd={nd}: phi={dict(zip(primes_list, coords))} W={W}"
                )
                break
