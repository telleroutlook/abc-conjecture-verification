...
print("=" * 95)
print(
    f"{'(a,b,c)':<18} {'type':<12} {'v_max':>6} {'R':>5} {'qual':>5} "
...
for a, b, _, label in all_cases:
    ...
    sq = (
        "sq"
        if (
            all(v == 1 for v in fa.values())
            and all(v == 1 for v in fb.values())
            and all(v == 1 for v in fc.values())
        )
        else "ns"
    )
    print(
        f"  ({a},{b},{c}){'':<5} [{sq}]{label[3:]:>8} {vmax:>6} {R:>5} {qual:>5.3f} "
...
for a, b, _, label in all_cases:
    ...
    if not ok:
        print(
            f"  FAIL: ({a},{b},{c}): |W_psi|={abs(res['W_psi'])} > omega*nd={omega * res['norm']}"
        )
        all_ok = False
