    ...
    for ap in range(-bound, bound+1):
        for aq in range(-bound, bound+1):
            for ar1 in range(-bound, bound+1):
                phi_r2 = ap + aq - ar1
                if abs(phi_r2) > bound: continue
                if aq == ap: continue  # degenerate
                if ap == 0 and aq == 0 and ar1 == 0: continue
    ...
    for p in primes[:8]:  # a = p (single prime)
        for q in primes:  # b = q (single prime)
            ...
            for r1 in primes:
                ...
                if r1 in (p, q) or r2 in (p, q): continue
                if len({p, q, r1, r2}) != 4: continue
                if p < r1:  # nd = r1 < q
...
for (p, q, r1, r2) in triples:
    ...
    status = "✓" if ok else "✗"
    dom = "r2" if pred==r2 and r2<=q and r2<=2*r1 else ("q" if pred==q else "2r1")
    print(f"  ({p},{q},{r1}*{r2}): min={norms[0]}=r1, 2nd={second}, "
          f"pred={pred}={dom}, r2={r2},q={q},2r1={2*r1} {status}")

print()
print("="*70)
print(f"F33 {'VERIFIED ✓' if all_ok else 'FAILS ✗'} for all tested type-(1,1,2) triples.")
