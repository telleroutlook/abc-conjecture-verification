    ...
    for fp in range(-bound, bound+1):
        for fq in range(-bound, bound+1):
            for fr1 in range(-bound, bound+1):
                ...
                if fq == fp: continue  # 退化（W=pq(fq-fp)=0）
...
# F10 预测和最小非退化向量分析
print(f"{'(p,q,r1,r2)':22}  {'4素数排序':18}  {'nd(F10)':>7}  {'暴力最小':>8}  {'最优φ':>18}  {'匹配'}")
print("-"*100)
...
for p, q, r1, r2 in triples[:10]:
    ...
    # 识别哪两个坐标非零
    nonzero = []
    if fp != 0: nonzero.append(f"φ_p={fp}(→ψ_p={p*fp})")
    if fq != 0: nonzero.append(f"φ_q={fq}(→ψ_q={q*fq})")
    if fr1 != 0: nonzero.append(f"φ_r1={fr1}(→ψ_r1={r1*fr1})")
    if fr2 != 0: nonzero.append(f"φ_r2={fr2}(→ψ_r2={r2*fr2})")
    sorted4 = sorted([p, q, r1, r2])
    norm = max(p*abs(fp), q*abs(fq), r1*abs(fr1), r2*abs(fr2))
    print(f"  ({p},{q},{r1},{r2}): φ={phi}, 非零={nonzero}, 范数={norm}")
    print(f"    4素数升序:{sorted4}, 最小两个:{sorted4[:2]}, nd={sorted([p,q,r1])[1]}")
