...
def f10_nd(a, b, c):
    ...
    gm = []
    if fa:
        gm.append(min(fa.keys()))
    if fb:
        gm.append(min(fb.keys()))
    if fc:
        gm.append(min(fc.keys()))
    gm.sort()
...
for c in range(6, 2000):
    ...
    b = c - 1
    fb = factorize(b)
    if any(v > 1 for v in fb.values()) or len(fb) != 3:
        continue
    if set(fb.keys()) & set(fc.keys()):
        continue
    R = math.prod(set(fb.keys()) | set(fc.keys()))
...
print("  Top 10 type (0,3,2) triples (c<=2000):")
...
print(
    "    OR the single-prime group's prime is not the nd (dominated by multi-prime groups)."
)
