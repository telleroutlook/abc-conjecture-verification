...
def nd_and_lambda2_brute(p, q, r, k, bound):
    ...
    for xp in range(-bound, bound + 1):
        for xq in range(-bound, bound + 1):
            xr = k * xp + xq
            if abs(xr) > bound: continue
            if xp == 0 and xq == 0 and xr == 0: continue
            if xq - xp == 0: continue
            nrm = max(p * abs(xp), q * abs(xq), r * abs(xr))
            if nrm > 0:
                norms.add(nrm)
    if not norms: return None, None
    sn = sorted(norms)
...
for (a, b, p, q, r, k) in triples:
    ...
    if l2_b == l2_f:
        if regime == "val": val_ok += 1
        else: pair_ok += 1
    else:
        if regime == "val": val_fail += 1
        else: pair_fail += 1
        print(f"  L2-FAIL ({a},{b}) {regime} k={k} q={q} r={r}: "
