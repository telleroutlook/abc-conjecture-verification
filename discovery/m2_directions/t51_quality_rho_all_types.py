    ...
    for na, nb, nc in types:
        ...
        for pa, pb, pc in triples:
            ...
            if s >= 1:
                ...
                grand_ok = False
                print(
                    f"    ✗ FAIL ({na},{nb},{nc}): {pa}+{pb}={pc}, q={q:.4f}, rho^{omega - 1}={rho_pow:.4f}, sum={s:.6f}"
                )
            if s > max_sum:
                max_sum = s
        ...
        print(
            f"  ({na},{nb},{nc}): {n:3d} triples, max q+ρ^{omega - 1} = {max_sum:.6f}  {status}"
        )
...
print(
    "  ω=3 type (1,1,1): PROVED as F31 (2-line proof, Lean-formalized as pasten_F31a)"
)
