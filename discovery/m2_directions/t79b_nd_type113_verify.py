        ...
        for phi_s in range(-search, search + 1):
            for phi_t in range(-search, search + 1):
                ...
                if phi_q == 0:
                    continue
                norm = max(
                    q * abs(phi_q), r * abs(phi_r), s * abs(phi_s), t * abs(phi_t)
                )
                if norm > 0 and norm < best:
                    best = norm
    return best
        ...
        for phi_s in range(-search, search + 1):
            for phi_t in range(-search, search + 1):
                ...
                if phi_p == 0:
                    continue
                norm = max(
                    p * abs(phi_p), r * abs(phi_r), s * abs(phi_s), t * abs(phi_t)
                )
                if norm > 0 and norm < best:
                    best = norm
    return best
...
for nn in range(2, LIMIT + 1):
    ...
    if len(f) == 1:
        p2 = list(f.keys())[0]
        prime_powers[nn] = (p2, f[p2])
    elif len(f) == 3:
        ps = sorted(f.keys())
        three_prime_c[nn] = (ps[0], ps[1], ps[2], f[ps[0]], f[ps[1]], f[ps[2]])

print(
    f"T79b: type (1,1,3) exact nd verification — smart bounded verifier (LIMIT={LIMIT})"
)
print("=" * 70)
...
for a, (p, k) in prime_powers.items():
    for b, (q, m) in prime_powers.items():
        ...
        if math.gcd(a, b) != 1:
            continue
        c = a + b
        if c not in three_prime_c:
            continue
        r, s, t, n1, n2, n3 = three_prime_c[c]
        if p in (r, s, t) or q in (r, s, t):
            continue
        key = (min(a, b), max(a, b))
        if key in seen:
            continue
...
for a, b, p, q, r, s, t, k, m, n1, n2, n3 in triples:
    ...
    if tight:
        ...
        for i, v in enumerate(bvals):
            if v == fval:
                branch_wins[branch_names[i]] += 1
    else:
        fail += 1
        print(
            f"FAIL: ({a},{b}) p={p}^{k} q={q}^{m} c={r}^{n1}*{s}^{n2}*{t}^{n3}: "
