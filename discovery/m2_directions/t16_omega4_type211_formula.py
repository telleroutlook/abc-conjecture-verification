...
for c in range(4, 301):
    for a in range(2, (c + 1) // 2 + 1):
        ...
        if ok:
            ...
            failed += 1
            print(
                f"  FAIL ({a},{b},{c}): in_lat={in_lattice}, nd={non_degen}, norm={norm_psi}(exp {expected_norm}), W={W:.1f}(exp {expected_W}), lat={lat_check}"
            )
...
print("  " + "-" * 50)
a6_cases = [
    (ratio, a, b, c, p1, p2, q, s, R)
    for ratio, a, b, c, p1, p2, q, s, R in ratios
    if a == 6
]
for ratio, a, b, c, p1, p2, q, s, R in sorted(a6_cases, key=lambda x: x[2])[:15]:
    print(
        f"  {str((a, b, c)):>14}  {q:>5}  {s:>5}  {R ** (1 / 3):>10.3f}  {ratio:>8.4f}"
    )
print()
...
print(
    f"  {'q':>6}  {'s':>6}  {'formula':>10}  {'numerical':>10}  {'q^{1/3}/6^{1/3}':>14}"
)
...
print(
    "        So p₁p₂|ψ_q| = q|p₂ψ_{p₁}+p₁ψ_{p₂}| ≥ q (min nonzero value of |p₂ψ_{p₁}+p₁ψ_{p₂}| is p₂≥p₁)."
)
print(
    "        So |ψ_q| ≥ q/p₁p₂·p₂ = q/p₁ or |ψ_{p₁}|,|ψ_{p₂}| ≥ 1 → ‖ψ‖_∞ ≥ min(|ψ_q|, q/...) ..."
)
