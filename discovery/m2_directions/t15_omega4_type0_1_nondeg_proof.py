...
for c in range(3, 201):
    for a in range(1, (c + 1) // 2 + 1):
        ...
        if ptype == (1, 1, 2):
            ...
            p_val = pa[0]
            q_val = pb[0]  # both singletons
            r_val = pc[0]
            s_val = pc[1]  # pc has 2 primes, sorted
            # Verify vector (p,0,r,0): psi_a=p, psi_b=0, psi_r=r, psi_s=0
            psi = {p_val: p_val, q_val: 0, r_val: r_val, s_val: 0}
        elif ptype == (0, 2, 2):
            # a=1 (no primes), b=pq, c=rs
            p_val = pb[0]
            q_val = pb[1]  # pb has 2 primes p<q
            r_val = pc[0]
            s_val = pc[1]  # pc has 2 primes r<s
            psi = {p_val: p_val, q_val: 0, r_val: r_val, s_val: 0}
        ...
        if ok:
            ...
            failed += 1
            print(
                f"  FAIL ({a},{b},{c}) type={ptype}: in_lat={in_lattice}, non_degen={non_degen}, norm_lt_degen={norm_lt_degen}"
            )
            print(
                f"    psi={psi}, norm={norm_psi}, degen_min={degen_min}, W={W}, lat={lat_check}"
            )
...
print(
    "    max(p,r) < s: for p,q odd: r=2,s=(pq+1)/2>p. For p=2: s>sqrt(2q+1)>2=p, r<q<s."
)
print("    => max(p,r) < min(q,s). QED.")
print()
...
print(
    "  HONEST SCOPE: Does not imply abc. Non-degenerate bound is max(p,r)/R^{1/3} ≤ C"
)
