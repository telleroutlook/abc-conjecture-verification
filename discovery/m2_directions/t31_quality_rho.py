...
print()
print(f"  {'type':>12}  {'n':>6}  {'max quality':>12}  {'at triple':>20}  {'max ρ':>8}  {'corr?'}")
print("  " + "-"*75)

bounded_types = {(1,1,1),(0,2,2),(1,1,2),(1,2,1),(1,2,2),(2,1,2),(2,2,1)}
...
for pt in sorted(type_qrho.keys(), key=lambda t: (-sum(t), t)):
    ...
    max_r, max_r_q, max_r_trip = max(data, key=lambda x: x[1])[1], max(data, key=lambda x: x[1])[0], max(data, key=lambda x: x[1])[2]
    ...
    print(f"  {str(pt):>12}  {n:>6}  {max_q:>12.6f}  {str(max_q_trip):>20}  {max_r:>8.5f}  {bmark}")
...
for pt in [(1,1,1),(1,1,2),(1,2,1)]:
    if pt not in type_qrho: continue
    data = type_qrho[pt]
    q_vals = [x[0] for x in data]
    r_vals = [x[1] for x in data]
    n = len(q_vals)
    mean_q = sum(q_vals)/n; mean_r = sum(r_vals)/n
...
for omega in [3,4,5]:
    ...
    print(f"  ω={omega}: max quality = {max_q:.6f} at {max_q_triple[2]} type {max_q_triple[3]}")
print()
print("Note: quality ≤ 1/2 for all squarefree triples (proved below):")
print("  R = abc ≥ c² for squarefree? No: R = abc = c*(ab), and ab ≥ 1 but ab can be = 1.")
print("  Actually a≥1, b≥1, c=a+b: R=abc. For a=1,b=c-1: R=c(c-1)≥c²-c > c (c>1).")
print("  For a=2,b=c-2: R=2(c-2)c. For a=p,b=q,c=r (primes): R=pqr≥2·3·5=30 but c can be small.")
...
print("  For type (0,2,2): max quality ~ 0.33 (NO correlation with ρ — ρ max at (1,14,15)")
