...
def make_triples_of_type(ma, mb, mc, limit=15):
    ...
    for pa_tuple in [primes[i : i + ma] for i in range(0, 30, 1)] if ma > 0 else [()]:
        ...
        if a > 500:
            break
        for pb_tuple in (
            [primes[j : j + mb] for j in range(0, 30, 1)] if mb > 0 else [()]
        ):
            if len(pb_tuple) != mb:
                continue
            if set(pa_tuple) & set(pb_tuple):
                continue
            ...
            for pc_tuple in pcs:
                if sorted(pc_tuple) != list(pc_tuple):
                    continue  # ensure sorted
                results.append(
                    (a, b, c, sorted(pa_tuple), sorted(pb_tuple), list(pc_tuple))
                )
