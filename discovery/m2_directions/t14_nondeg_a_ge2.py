        ...
        det_L = lattice_det_val(
            {
                ...
                for p, c in zip(
                    ...
                    [
                        sum(
                            fa.get(p, 0) * (max(primes_set) * max(primes_set) // p)
                            for _ in [0]
                        )
...
print(
    f"  Results for a=1  (Pa empty):   {n_tested_a1} triples, {n_degen_a1} degenerate"
)
print(
    f"  Results for a>=2 (Pa nonempty): {n_tested_a2} triples, {n_degen_a2} degenerate"
)
print()
...
print(
    "    p1*psi_{p2} + p2*psi_{p1} = 0, minimal solution |psi|_inf = min(p1,p2) (small)."
)
...
print(
    "  Any solution needs the weighted sum over b-primes to equal weighted sum over a-primes."
)
...
print()
print(
    "  a=1  : degenerate shortest vectors EXIST; non-degen min <= 2*R^{1/2} (empirical)"
)
print(
    "  a>=2 : no degenerate shortest vectors found; Corollary C gives non-degen bound"
)
print()
